...
func (h *ImportHandler) generateSchoolCredentialPDF(ctx context.Context, schoolUUID uuid.UUID) ([]byte, string, error) {
	...
	// Get NISN users filtered by school (only users who belong to teams at this school)
	filter := model.UserNISNFilter{SchoolID: &schoolUUID}
	users, err := h.userRepo.ListByNISN(ctx, filter)
	if err != nil {
		return nil, "", fmt.Errorf("gagal mengambil data pengguna")
	}

	// Cache games indexed by ID
	allGames, _ := h.gameRepo.List(ctx)
	gameByID := make(map[uuid.UUID]*model.Game, len(allGames))
	for _, g := range allGames {
		gameByID[g.ID] = g
	}

	// Batch-fetch all teams for this school
	schoolTeams, _, _ := h.teamRepo.List(ctx, model.TeamFilter{SchoolID: &schoolUUID, Page: 1, Limit: 1000})

	// Build team lookup: teamID -> team
	teamByID := make(map[uuid.UUID]*model.Team, len(schoolTeams))
	for _, t := range schoolTeams {
		teamByID[t.ID] = t
	}

	// Build userID -> all team memberships at this school
	type userTeamInfo struct {
		Team       *model.Team
		MemberRole string
	}
	userTeams := make(map[uuid.UUID][]userTeamInfo, len(users))
	for _, u := range users {
		members, err := h.teamMemberRepo.FindByUser(ctx, u.ID)
		if err != nil {
			continue
		}
		for _, m := range members {
			if t, ok := teamByID[m.TeamID]; ok {
				...
				userTeams[u.ID] = append(userTeams[u.ID], userTeamInfo{Team: t, MemberRole: role})
			}
		}
	}
	...
	for _, u := range users {
		...
		// Generate one card per team membership (player may be in multiple teams)
		for _, info := range infos {
			...
			if gameDisplay == "" {
				if g, ok := gameByID[team.GameID]; ok {
					gameDisplay = g.Name
				}
			} else {
...
// ExportAllCredentialsPDFZip generates credential PDFs for all schools and returns a ZIP archive.
// GET /admin/import/credentials/pdf/all
func (h *ImportHandler) ExportAllCredentialsPDFZip(c *fiber.Ctx) error {
	// Get all schools
	schools, _, err := h.schoolRepo.List(c.Context(), model.SchoolFilter{Page: 1, Limit: 1000})
	if err != nil {
		return response.BadRequest(c, "Gagal mengambil daftar sekolah")
	}
	...
	for _, school := range schools {
		pdfBytes, schoolName, err := h.generateSchoolCredentialPDF(c.Context(), school.ID)
		if err != nil {
			continue // skip schools with no participants
		}
	...
	if err := zipWriter.Close(); err != nil {
		return response.BadRequest(c, "Gagal membuat file ZIP")
	}

	if generated == 0 {
		return response.BadRequest(c, "Tidak ada sekolah dengan peserta")
	}

	c.Set("Content-Type", "application/zip")
	c.Set("Content-Disposition", `attachment; filename="Kredensial_Semua_Sekolah.zip"`)
...
<div class="body">
<form method="POST" action="/api/v1/public/credentials/` + token + `">
...
func drawCredentialCard(pdf *gofpdf.Fpdf, x, y, w, h float64, card credentialCard) {
	...
	pdf.SetXY(x+4, contentY)
	pdf.CellFormat(w-8, 3, "Simpan kartu ini dan jaga kerahasiaannya", "", 0, "L", false, 0, "")
}
