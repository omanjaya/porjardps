...
interface Props {
  ...
  /** Base href for breadcrumb — e.g. "/dashboard", "/coach", "/referee" */
  baseHref: string
  /** Base label for breadcrumb — e.g. "Dashboard", "Coach" */
  baseLabel?: string
  /** Optional post-save redirect intent (player onboarding use case) */
  fromIntent?: string | null
}

export function ProfilePageContent({ Layout, baseHref, baseLabel = 'Dashboard', fromIntent = null }: Props) {
  ...
  // Load team info (player only)
  useEffect(() => {
    if (!isPlayer) return
    api
      .get<{ team: ProfileTeamInfo | null }>('/player/dashboard')
      .then((res) => { if (res.team) setTeamInfo(res.team) })
      .catch(() => {})
  }, [isPlayer])
  ...
  const roleLabel = (() => {
    switch (user?.role) {
      ...
      case 'coach': return 'Guru Pembina'
  ...
  return (
    <Layout>
      <PageHeader
        title="Profil Saya"
        breadcrumbs={[{ label: baseLabel, href: baseHref }, { label: 'Profil' }]}
      />
      ...
      <div className="max-w-xl space-y-6">
          ...
          <div className="relative px-6 pb-5">
            <div className="flex flex-col items-center sm:flex-row sm:items-end gap-4 -mt-10">
              ...
              <div className="text-center sm:text-left flex-1 min-w-0 pb-1">
                ...
                <p className="mt-0.5 text-xs text-stone-400 dark:text-zinc-500">{user?.email}</p>
                <div className="mt-2 flex flex-wrap items-center justify-center sm:justify-start gap-2">
                  {isPlayer && teamInfo && (
                    <Link
                      href="/dashboard/teams"
                      className="inline-flex items-center gap-1.5 rounded-full border border-blue-200 dark:border-blue-800/50 bg-blue-50 dark:bg-blue-950/30 px-2.5 py-0.5 text-[11px] font-medium text-blue-700 dark:text-blue-300 hover:border-blue-300 transition-colors"
                    >
        ...
        {/* Profile completion progress (player only) */}
        {isPlayer && (
          <div className="rounded-xl border border-stone-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-4 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-semibold text-stone-700 dark:text-zinc-300 flex items-center gap-1.5">
                <CheckCircle size={14} weight={completion.filled === completion.total ? 'fill' : 'regular'} className={completion.filled === completion.total ? 'text-green-500' : 'text-stone-400'} />
                Kelengkapan Profil
              </p>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                />
                <button type="button" onClick={() => setShowCurrent(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-600 dark:hover:text-zinc-300">
            ...
            <div>
              ...
              <div className="relative">
                <Input
                  ...
                  className="border-stone-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 dark:text-zinc-100 pr-10 focus:border-esi-red"
                />
                <button type="button" onClick={() => setShowNew(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-600 dark:hover:text-zinc-300">
          ...
          <div className="rounded-xl border border-stone-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-6 shadow-sm">
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer items-center rounded-full transition-colors duration-200 disabled:opacity-50 ${pushSubscribed ? 'bg-esi-red' : 'bg-stone-300 dark:bg-zinc-600'}`}
              >
                <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform duration-200 ${pushSubscribed ? 'translate-x-6' : 'translate-x-1'}`} />
              </button>
            </div>
          </div>
        )}
