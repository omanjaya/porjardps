      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:px-6 lg:px-8">
        <Link href="/" className="flex items-center gap-2">
          ...
          <div className="ml-1 flex flex-col leading-none">
            <span className="text-sm font-bold tracking-tight" style={{ color: RED }}>ESI</span>
            <span className="text-[10px] font-medium text-stone-500 tracking-wider">KOTA DENPASAR</span>
          </div>
        </Link>
        <nav className="flex items-center gap-1">
          {NAV_ITEMS.map(item => (
            <Link key={item.href} href={item.href} className="px-3 py-1.5 text-[13px] font-semibold text-stone-600 dark:text-zinc-400 rounded-lg transition hover:bg-stone-100 dark:hover:bg-zinc-800 hover:text-stone-900 dark:hover:text-zinc-100">
              {item.label}
            </Link>
          ))}
        </nav>
        <div className="flex items-center gap-2">
          ...
          <Link href="/login" className="text-[13px] font-medium text-stone-500 transition hover:text-stone-800 dark:text-zinc-400 dark:hover:text-zinc-200 hidden sm:block px-2">
            Masuk
          </Link>
          <Link href="/register" className="rounded-lg px-4 py-1.5 text-[13px] font-bold text-white transition hover:brightness-110" style={{ background: RED }}>
            Daftar
          </Link>
        </div>
      </div>
