...
export function CoachLayout({ children }: CoachLayoutProps) {
  ...
  return (
    <div className="flex min-h-[100dvh] bg-porjar-bg">
      ...
      <aside
        className={cn(
          'fixed inset-y-0 left-0 z-50 flex w-60 flex-col border-r border-porjar-border bg-white transition-transform duration-300 ease-in-out lg:static lg:translate-x-0',
      ...
      <div className="flex flex-1 flex-col">
        ...
        <header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b border-porjar-border bg-white px-4">
          ...
          <div className="ml-auto flex items-center gap-3">
            <NotificationBell />
            <span className="text-sm text-porjar-muted">{user?.full_name ?? 'Guru Pembina'}</span>
            <Button
              variant="ghost"
              size="sm"
